...
const askModel = () => __awaiter(void 0, void 0, void 0, function* () {
    const answer = readline_sync_1.default.question(`\n[whisper-node-ts] Enter model name (e.g. 'base.en') or 'cancel' to exit\n(ENTER for base.en): `);
    if (answer === "cancel") {
        console.log("[whisper-node-ts] Exiting model downloader. Run again with: 'npx whisper-node-ts download'");
        process.exit(0);
    }
    // user presses enter
    else if (answer === "") {
        console.log("[whisper-node-ts] Going with", constants_1.DEFAULT_MODEL);
        return constants_1.DEFAULT_MODEL;
    }
    else if (!MODELS_LIST.includes(answer)) {
        console.log("\n[whisper-node-ts] FAIL: Name not found. Check your spelling OR quit wizard and use custom model.");
        // re-ask question
...
`);
            ...
            if (!shelljs_1.default.which("./download-ggml-model.sh")) {
                throw "whisper-node-ts downloader is not being run from the correct path! cd to project root and run again.";
            }
            ...
            console.log("[whisper-node-ts] Attempting to compile model...");
