...
const modelPathOrName = (mn, mp) => {
    if (mn && mp)
        ...
        console.log("[whisper-node-ts] No 'modelName' or 'modelPath' provided. Trying default model:", constants_1.DEFAULT_MODEL, "\n");
        ...
        if (!(0, fs_1.existsSync)(modelPath)) {
            // throw `'${mn}' not downloaded! Run 'npx whisper-node-ts download'`;
            throw `'${constants_1.DEFAULT_MODEL}' not downloaded! Run 'npx whisper-node-ts download'\n`;
        }
        return modelPath;
    }
    // modelpath
    ...
    // modelname
        ...
        if (!(0, fs_1.existsSync)(modelPath)) {
            throw `'${mn}' not found! Run 'npx whisper-node-ts download'`;
        }
        return modelPath;
    }
    else if (mn)
